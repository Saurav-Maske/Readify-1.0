...
export function CommentThread({ comment, onReply, depth = 0 }: CommentThreadProps) {
  ...
  return (
    <div className={depth > 0 ? 'ml-8 mt-3 border-l border-gray-100 dark:border-gray-800 pl-4' : 'mt-3'}>
      <div className="flex items-start gap-2.5">
        <Avatar name={comment.author.name} src={comment.author.avatarUrl} size="sm" />
        <div className="min-w-0 flex-1">
          <div className="rounded-2xl bg-background dark:bg-background-dark px-3 py-2">
            <div className="flex flex-wrap items-baseline gap-x-2">
              <span className="text-sm font-semibold text-text dark:text-text-dark">{comment.author.name}</span>
              <span className="text-xs text-textSecondary dark:text-textSecondary-dark">{formatRelativeTime(comment.createdAt)}</span>
            </div>
            <p className="mt-0.5 whitespace-pre-wrap text-sm text-text dark:text-text-dark">{comment.content}</p>
          </div>

          <button
            type="button"
            onClick={() => setIsReplying((open) => !open)}
            className="mt-1 px-3 text-xs font-semibold text-textSecondary dark:text-textSecondary-dark transition-colors duration-150 hover:text-primary"
          >
            Reply
          </button>
